...
export function IsraelAddressSelect({ label, value, onChange, placeholder, city }: IsraelAddressSelectProps) {
  ...
  return (
    <div className="relative text-right">
      ...
      {open && (loading || results.length > 0) ? (
        <ul className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded border border-black/10 bg-white shadow-lg">
          {loading ? <li className="px-3 py-2 text-sm text-black/40">מחפש...</li> : null}
          {results.map((record) => (
            <li key={record._id}>
              <button
                type="button"
                ...
                className="block w-full px-3 py-2 text-right text-sm hover:bg-brand-soft/40"
              >
